...
my $split_root = $data_root;
my $taskCmd = "java -cp /soe/manazhao/projects/EgoShishang/egoshishang-Jan03.jar:\$CLASSPATH com.egoshishang.amazon.ProductDetailedPageFetcher ";
...
for(my $i = 0; $i < $split_cnt; $i++)
{
	...
	my $remote_cmd = "ssh $cur_host  '. ~/.bashrc; nohup $taskCmd $in_fp $output_fp 1 > $log_fp 2>&1 &' ";
	print $remote_cmd."\n";
	`$remote_cmd`;
